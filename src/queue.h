...
{
public:
    q_policy1(size_t size) noexcept : m_capacity{ size } { }
    ...
    size_t previous_rear()  const noexcept { return (m_rear  - 1)  % (m_capacity); }
    size_t previous_front() const noexcept { return (m_front - 1)  % (m_capacity); }

    size_t increase_rear()          noexcept { return ::std::exchange(m_rear , next_rear());        }
    size_t decrease_rear()          noexcept { return ::std::exchange(m_rear , previous_rear());    }
...
{
public:
    q_policy2(size_t size) noexcept : m_capacity{ size } { }

    bool full()  const noexcept { return size() == m_capacity; }
    bool empty() const noexcept { return m_rear == m_front + 1; }

    size_t next_rear()      const noexcept { return (m_rear  + 1)  % (m_capacity + 1); }
    size_t next_front()     const noexcept { return (m_front + 1)  % (m_capacity + 1); }
    size_t previous_rear()  const noexcept { return (m_rear  - 1)  % (m_capacity + 1); }
    size_t previous_front() const noexcept { return (m_front - 1)  % (m_capacity + 1); }

    size_t increase_rear()          noexcept { return ::std::exchange(m_rear , next_rear());        }
    size_t decrease_rear()          noexcept { return ::std::exchange(m_rear , previous_rear());    }
    size_t increase_front()         noexcept { return ::std::exchange(m_front, next_rear());        }
    size_t decrease_front()         noexcept { return ::std::exchange(m_front, previous_front());   }
    size_t size()             const noexcept { return m_size; }
    void increase_size()            noexcept { ++m_size; }
    void decrease_size()            noexcept { --m_size; }
...
public:
    ...
    { 
        ...
        const size_t old_rear = policy_.increase_rear();   

        if (auto& entry = s_[old_rear]; entry != 0)
    ...
    { 
        ...
        const size_t old_front = policy_.decrease_front(); 

        if (auto& entry = s_[old_front]; entry != 0)
            throw ::std::logic_error{ policy_.describe() };
        else entry = 1;
    ...
    { 
        ...
        const size_t old_rear = policy_.decrease_rear(); 

        if (auto& entry = s_[old_rear]; entry != 1)
            throw ::std::logic_error{ policy_.describe() };
        else entry = 0;
    ...
    { 
        ...
        const size_t old_front = policy_.increase_front(); 

        if (auto& entry = s_[old_front]; entry != 1)
            throw ::std::logic_error{ policy_.describe() };
        else entry = 0;
